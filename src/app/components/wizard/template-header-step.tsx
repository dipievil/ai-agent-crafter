...
export default function TemplateHeaderStep({
  selectedToolId,
  selectedType,
  entityName,
  entityDescription,
  ...
  const schema = useMemo(() => {
    ...
    return service.buildForm(selectedToolId, selectedType, "header", entityName, entityDescription);
  }, [entityDescription, entityName, selectedToolId, selectedType, tAll]);
  ...
  return (
    <>
      ...
      <div className="mt-4 flex flex-col gap-5 text-left">
        {fields.length === 0 ? (
          <p className="text-sm text-foreground/80">{schema.meta.message ?? t("noHeader")}</p>
        ) : (
          fields.map((field) => {
