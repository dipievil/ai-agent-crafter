...
- Check for `development.md` for specific instructions on how to contribute to the project.
...
## Development Instructions
 - For new steps, create a new file under `src/app/components/wizard/` with the component logic and a corresponding `*.types.ts` file for TypeScript types. 
 - Each step will add a new entry on the SummarySection.
...
- Always review for not used labels on translations and remove them if they are not used anymore. The translations are located in `messages/en.json` and accessed via the `useTranslations` hook.
